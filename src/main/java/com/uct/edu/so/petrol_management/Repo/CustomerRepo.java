...
public interface CustomerRepo extends JpaRepository<CustomerModel,Integer> {
    ...
    @Query(value = "SELECT c_id, cname,ifnull(petroll_type.p_type,0)as p_type, ifnull(transection.trns_id,0)as trns_id,ifnull(round(((numofliter*priceper_liter)- transection.amountpaid),2),0) as amountRemaining,IFnull(transection.numofliter*transection.priceper_liter,0)as totalAmount,ifnull(transection.amountpaid,0)as  amountpaid,ifnull(transection.numofliter,0) as numofliter,customer.begining_balance\n" +
